...
export function RecapHero({ hackathon, entries, sections }: Props) {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 via-indigo-50/40 to-slate-50 border border-gray-200 rounded-2xl p-8 mb-10 text-center">
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-6">
        {participantCount > 0 && (
          <div className="bg-white border border-gray-200 rounded-xl px-5 py-3 text-center min-w-[80px] shadow-sm">
            <p className="font-extrabold text-2xl">{participantCount}</p>
            <p className="text-xs text-gray-400 mt-0.5">참가팀</p>
          </div>
        )}
        {maxScore != null && (
          <div className="bg-white border border-brand/20 rounded-xl px-5 py-3 text-center min-w-[80px] shadow-sm">
            <p className="font-extrabold text-2xl text-brand">{maxScore}</p>
            <p className="text-xs text-gray-400 mt-0.5">최고점</p>
          </div>
        )}
        {totalPrize > 0 && (
          <div className="bg-white border border-yellow-200 rounded-xl px-5 py-3 text-center min-w-[80px] shadow-sm">
